...
import { ArrowLeft, MapPin, Building2, User, TrendingUp, AlertTriangle, Flag, StickyNote, Pencil, Shield, Ban } from "lucide-react";
...
import { MOCK_ADMIN_ASSESSMENTS, type ScoreBand, type AdminAssessmentStatus } from "../../../admin-data/mockAdminAssessments";
import { formatAssessmentDate } from "../../../admin-lib/dateTime";
import { canEditAssessment } from "../../../admin-lib/permissions";
import { logAssessmentViewed, logScoreBandUpdated, logStatusUpdated, logNotesUpdated } from "../../../admin-lib/audit";
...
export default function AdminAssessmentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [assessmentData, setAssessmentData] = useState(() =>
    MOCK_ADMIN_ASSESSMENTS.find((a) => a.id === id) ?? null
  );
  ...
  if (user && typeof window !== "undefined") {
    const viewedKey = `viewed_${assessmentData.id}_${user.id}`;
    if (!sessionStorage.getItem(viewedKey)) {
      sessionStorage.setItem(viewedKey, "1");
      logAssessmentViewed(user, assessmentData.loanRef);
    }
  }

  const handleSave = (updates: { scoreBand: ScoreBand; status: AdminAssessmentStatus; notes: string }) => {
    if (!user) return;

    const prev = assessmentData;

    // Log each changed field
    if (updates.scoreBand !== prev.scoreBand) {
      logScoreBandUpdated(user, prev.loanRef, prev.scoreBand, updates.scoreBand);
    }
    if (updates.status !== prev.status) {
      logStatusUpdated(user, prev.loanRef, prev.status, updates.status);
    }
    if (updates.notes !== prev.notes) {
      logNotesUpdated(user, prev.loanRef, prev.notes, updates.notes);
    }

    // Update local state
    setAssessmentData({ ...prev, ...updates });
    setEditOpen(false);
  };
