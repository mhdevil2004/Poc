...
import type { AdminAssessment, ScoreBand, AdminAssessmentStatus } from "../../admin-data/mockAdminAssessments";
import { SCORE_BAND_ORDER, SCORE_BAND_DEFINITIONS } from "../../admin-data/mockAdminAssessments";
import { useTranslation } from "@/i18n";

interface AssessmentEditModalProps {
  assessment: AdminAssessment;
  onClose: () => void;
  onSave: (updates: { scoreBand: ScoreBand; status: AdminAssessmentStatus; notes: string }) => void;
}
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      <div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-white rounded-2xl shadow-2xl flex flex-col">
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-100 bg-slate-50/50 rounded-b-2xl">
          ...
          <button
            onClick={handleSave}
            className="px-5 py-2 text-sm font-bold text-white bg-violet-600 hover:bg-violet-700 rounded-xl shadow-sm shadow-violet-500/20 transition-colors flex items-center gap-2"
          >
