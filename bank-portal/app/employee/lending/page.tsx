...
import { Search, ArrowUpRight, CreditCard, CheckCircle2 } from "lucide-react";
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/70">
                ...
                <th className="text-left px-4 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-wider hidden lg:table-cell">Officer</th>
                ...
                return (
                  <tr key={loan.loanId} className="hover:bg-slate-50/60 transition-colors">
                    <td className="px-6 py-3.5">
                      <span className="text-slate-700 font-bold text-xs">{loan.loanId}</span>
                    </td>
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 max-w-lg w-full p-6 animate-in fade-in zoom-in-95 duration-150">
            <div className="flex justify-between items-start mb-4 pb-3 border-b border-slate-100">
              <div>
                ...
                <p className="text-xs text-slate-500">Applicant: {selectedLoanForAction.customerName} • {formatIDR(selectedLoanForAction.amount)}</p>
              </div>
              <button
                  ...
                  <button
                    onClick={() => {
                      if (selectedLoanForAction) {
                        toast(`🔍 Full Fraud Scan triggered for ${selectedLoanForAction.loanId}`, { icon: "🛡️" });
                        setSelectedLoanForAction(null);
                      }
                    }}
