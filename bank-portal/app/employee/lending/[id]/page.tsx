...
export default function LoanDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const customer = MOCK_CUSTOMERS.find((c) => c.customerId === loanData.customerId);
  ...
  const handleApprove = () => {
    ...
    toast.success(`Application ${loanData.loanId} approved by ${employee?.name || "Underwriter"}`);
  };

  const handleReject = () => {
    setCurrentStatus("Rejected");
    toast.error(`Application ${loanData.loanId} rejected.`);
  };

  const handleFlagRisk = () => {
    setCurrentStatus("Manual Review Required");
    toast(`Application ${loanData.loanId} flagged for Risk Officer Audit.`, { icon: "⚠️" });
  };
        ...
        {role === "Risk Officer" && (
          <div>
            ...
            <div className="flex flex-wrap items-center gap-3 pt-3 border-t border-slate-100">
              ...
              <button
                onClick={() => toast.success("Risk exception approved by Risk Officer")}
                className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-xl transition-all shadow-sm flex items-center gap-1.5"
              >
                <CheckCircle2 className="w-4 h-4" />
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-sm font-bold text-slate-900">Administrator Control Console</h3>
                  <p className="text-xs text-slate-500">Full administrative authority over applications, officers, and system rules</p>
                </div>
              </div>
            ...
            <div className="flex flex-wrap items-center gap-3 pt-3 border-t border-slate-100">
              ...
              <button
                onClick={() => toast.success("Assigned officer updated")}
              ...
              <button
                onClick={() => toast.success("Assessment rules overridden")}
                className="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold rounded-xl transition-all shadow-sm flex items-center gap-1.5"
              >
                <Sliders className="w-4 h-4" />
